...
// https://rtfm.co.ua/ru/c-sokety-i-primer-modeli-client-server/
...
// https://nischal.info.np/time-of-day-tcp-client-server-model-example-using-sockets
// https://www.thiyagaraaj.com/tutorials/unix-network-programming-example-programs/tcp-chat-programming-in-unix-using-c-programmin
// https://en.ittrip.xyz/c-language/c-epoll-echo-server
...
int create_server_socket(void) {
    ...
    if (socket_fd == -1) {
        PRINT_ERROR("Socket error: %s\n", strerror(errno));
        return (-1);
    }
    PRINT_DEBUG("Created server socket fd: %d\n", socket_fd);

    // Bind socket to address and port
    status = bind(socket_fd, (struct sockaddr *)&sa, sizeof sa);
    if (status != 0) {
        PRINT_ERROR("Bind error: %s\n", strerror(errno));
        return (-1);
    }
    PRINT_DEBUG("Bound socket to localhost port %d\n", PORT);
...
void accept_new_connection(int server_socket, struct pollfd **poll_fds, int *poll_count, int *poll_size)
{
    ...
    PRINT_DEBUG("Accepted new connection on client socket %d.\n", client_fd);

    memset(&msg_to_send, '\0', sizeof msg_to_send);
    sprintf(msg_to_send, "Welcome. You are client fd [%d]", client_fd);
    status = send(client_fd, msg_to_send, strlen(msg_to_send), 0);
    if (status == -1) {
        PRINT_ERROR("Send error to client %d: %s\n", client_fd, strerror(errno));
    }
}
...
void read_data_from_socket(int i, struct pollfd **poll_fds, int *poll_count, int server_socket)
{
    ...
    if (bytes_read <= 0) {
        if (bytes_read == 0) {
            PRINT_DEBUG("[%d] Client socket closed connection.\n", sender_fd);
        }
        else {
        ...
        PRINT_DEBUG("[%d] Got message: %s\n", sender_fd, buffer);

        memset(&msg_to_send, '\0', sizeof msg_to_send);
        sprintf(msg_to_send, " [%d] says: %s\n", sender_fd, buffer);
        for (int j = 0; j < *poll_count; j++) {
            ...
            if (dest_fd != server_socket && dest_fd != sender_fd) {
                ...
                if (status == -1) {
                    PRINT_ERROR("Send error to client fd %d: %s\n", dest_fd, strerror(errno));
                }
            }
        }
    }
...
int main(void)
{
    ...
    if (server_socket == -1) {
		PRINT_ERROR("Create Server Socket error\n");
        return (1);
    }

    // Listen to port via socket
    PRINT_DEBUG("Listening on port %d\n", PORT);
    status = listen(server_socket, 10);
    if (status != 0) {
        PRINT_ERROR("Listen error: %s\n", strerror(errno));
        return (3);
    }
    ...
    while (1) { // Main loop
        ...
        for (int i = 0; i < poll_count; i++) {
            ...
            PRINT_DEBUG("[%d] Ready for I/O operation\n", poll_fds[i].fd);
